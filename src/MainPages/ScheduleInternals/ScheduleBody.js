  ...
  render() {
    ...
    return (
      <div>
        ...
        { awardYears.includes(this.state.year) &&  <div>
            <Sub>
        ...
        {Object.keys(awards[this.state.year]).map(post => {
            return (<div>
<AwardPannel title =  {awards[this.state.year][post].awardName} index =  {awards[this.state.year][post].id}>
   ...
   {awardYears.includes(this.state.year) && scheduleYears.includes(this.state.year) && <div className = "someSpacing"></div>}
